...
// Запускается функция pow, с аргументами x=2, n=3. Эти переменные хранятся в контексте выполнения, схематично изображённом ниже:
//
// Контекст: { x: 2, n: 3, строка 1 } Выполнение в этом контексте продолжается, пока не встретит вложенный вызов в строке 3.
// pow(2, 2)
...
//
// Контекст: { x: 2, n: 3, строка 3 }Контекст: { x: 2, n: 2, строка 1 } Обратим внимание, что контекст включает в себя не только переменные, но и место в коде, так что когда вложенный вызов завершится -- можно будет легко вернуться назад.
...
//
// Контекст: { x: 2, n: 3, строка 3 }Контекст: { x: 2, n: 2, строка 3 }Контекст: { x: 2, n: 1, строка 1 } На текущий момент в стеке уже два старых контекста.
// Выход из pow(2, 1).
...
//
// Контекст: { x: 2, n: 3, строка 3 }Контекст: { x: 2, n: 2, строка 3 } Возобновляется обработка внешнего вызова `pow(2, 2)`.
// Выход из pow(2, 2).
// …И теперь уже pow(2, 2) может закончить свою работу, вернув 4. Восстанавливается контекст предыдущего вызова:
//
// Контекст: { x: 2, n: 3, строка 3 } Возобновляется обработка внешнего вызова `pow(2, 3)`.
// Выход из pow(2, 3).
